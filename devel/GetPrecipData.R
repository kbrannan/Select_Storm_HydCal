...
if(length(ls(all.names=TRUE)) > 0) dont.del <- paste0("(",ls(all.names=TRUE),")",collapse="|")
...
df.daily.precip <- cbind(df.daily.precip,
                         date=as.POSIXct(strptime(df.daily.precip[,1], 
                                                  format="%Y-%m-%d")))
# clean up
if(1*exists("dont.del")==0) rm(list=ls(all.names=TRUE)[-grep("df.daily.precip",ls(all.names=TRUE))])
if(1*exists("dont.del")==1) rm(list=ls(all.names=TRUE)[-grep(paste0("(df.daily.precip)|",dont.del),ls(all.names=TRUE))])
