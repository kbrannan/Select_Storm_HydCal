...
for (ii in 1:length(chr.prec.files)) {
  ...
  tmp.df.precip <- data.frame(
    date = format(as.Date(substr(tmp.prec.file, start = 1, stop = 11)), 
                  fmt="%Y-%m-%d"),
    prec = as.numeric(do.call(rbind,
                              lapply(tmp.prec.file, 
                                     function(x) substr(x, start = 15, 
                                                        stop = nchar(x))))),
    src = chr.prec.files[ii], stringsAsFactors = TRUE)
  if (exists("df.daily.precip") == FALSE) {
    df.daily.precip <- summaryBy( .~ date + src, data = tmp.df.precip, 
                                 FUN = sum)
  } else {
    df.daily.precip <- rbind(df.daily.precip,
                             summaryBy( .~ date + src, data = tmp.df.precip, 
                                       FUN = sum))
  }
