...
df.daily.precip.max.stations <- cbind(summaryBy(prec.sum ~ date_org, df.daily.precip, 
                                          FUN=max), 
...
plotToFile(as.numeric(unique(format(df.flow.est.clp$date,format="%Y"))),
           lst.pot.strm.5, df.flow.est.clp$date, 
...
df.pot.00.sum <- summaryBy(prec.sum.max ~ strm.num, data=df.pot.00, FUN=sum)

keep.strm.nums <- as.numeric(df.pot.00.sum$strm.num[
  df.pot.00.sum$prec.sum.max > 0])
...
df.pot.00.p.max <- summaryBy(prec.sum.max ~ strm.num, data=df.pot.00, FUN=max)

df.pot.00.f.max <- summaryBy(flow ~ strm.num, data=df.pot.00, FUN=max)

junk <- data.frame(strm.num=unique(df.pot.00$strm.num), 
                   date.p=as.POSIXct("1967-07-02"), date.f=as.POSIXct("1967-07-02"), 
                   diff=0)
...
plotIndvToFile(tmp.lst.pot.strm=lst.pot.strm.5.single.gt0.fafp.pgt01, 
               ...
               df.flow.est.clp$mean_daily_flow, 
               df.daily.precip.max.stations[ , 2],
               out.file=paste0(getwd(),"/figures/
                               strmInvdPlots5singlegt0fafppgt01.pdf"))
...
lst.pot.strm.sum$pot.strm <- lst.pot.strm.5.single.gt0.fafp.pgt01$pot.strm[
  lst.pot.strm.5.single.gt0.fafp.pgt01$pot.strm$strm.num %in% df.months.01$strm.num, ]
...
plotIndvToFile(tmp.lst.pot.strm=lst.pot.strm.fal, 
               df.flow.est.clp$date, 
               df.flow.est.clp$mean_daily_flow, 
               df.daily.precip.max.stations[ , 2],
...
plotIndvToFile(tmp.lst.pot.strm=lst.pot.strm.win, 
               df.flow.est.clp$date, 
               df.flow.est.clp$mean_daily_flow, 
               df.daily.precip.max.stations[ , 2],
...
plotIndvToFile(tmp.lst.pot.strm=lst.pot.strm.spr, 
               df.flow.est.clp$date, 
               df.flow.est.clp$mean_daily_flow, 
               df.daily.precip.max.stations[ , 2],
...
lst.pot.strm.xse$pot.strm <- lst.pot.strm.5.single.gt0.fafp$pot.strm[
  lst.pot.strm.5.single.gt0.fafp$pot.strm$strm.num %in% df.xse, ]
...
plotIndvToFile(tmp.lst.pot.strm=lst.pot.strm.xse, 
               df.flow.est.clp$date, 
               df.flow.est.clp$mean_daily_flow, 
               df.daily.precip.max.stations[ , 2],
...
## final storms
df.strm.summary <- table.me(yr.b=NULL, z=lst.pot.strm.5.single.gt0.fafp.pgt01)

df.strm.summary.chr <- data.frame(Number=sprintf("%i",df.strm.summary$strm.num),
                                  Start=format(df.strm.summary$date.bgn,
                                               format="%Y-%m-%d"),
                                  End=format(df.strm.summary$date.end,
                                               format="%Y-%m-%d"),
...
pdf(file=paste0(getwd(),"/tables/strm_sum.pdf"))
